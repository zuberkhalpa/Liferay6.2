...
String jp2MimeType = (String) fileEntry.getExpandoBridge().getAttribute("jp2MimeType");
String jp2FileEntryId = (String) fileEntry.getExpandoBridge().getAttribute("jp2FileEntryId");

FileEntry jp2FileEntry = null;
...
if(jp2MimeType.equalsIgnoreCase("image/jp2")) { 
	jp2FileEntry = DLAppServiceUtil.getFileEntry(Long.valueOf(jp2FileEntryId));
	if (portletDisplay.isWebDAVEnabled()) {
		webDavUrl = DLUtil.getWebDavURL(themeDisplay, jp2FileEntry.getFolder() , jp2FileEntry);
	}
}

System.out.println("jp2MimeType:=>"+jp2MimeType);

System.out.println("jp2FileEntryId:=>"+jp2FileEntryId);
						...
						<span class="download-document">
							<c:if test="<%= DLFileEntryPermission.contains(permissionChecker, fileEntry, ActionKeys.VIEW) %>">
								<% if(jp2MimeType.equalsIgnoreCase("image/jp2")) { %>
									...
									url="<%= DLUtil.getPreviewURL(jp2FileEntry,jp2FileEntry.getFileVersion(), themeDisplay, StringPool.BLANK) %>"
								/>
								<%} else {%>
								<liferay-ui:icon
									image="download"
						...
						<div class="lfr-asset-field url-file-container hide">
							<aui:field-wrapper name="url">
								<% if(jp2MimeType.equalsIgnoreCase("image/jp2")) { %>
									...
									id="url"
									url="<%= DLUtil.getPreviewURL(jp2FileEntry, jp2FileEntry.getFileVersion(), themeDisplay, StringPool.BLANK, false, true) %>"
								/>
								<%} else { %>
								<liferay-ui:input-resource
									id="url"
			...
			fileEntryButtonGroup.push(
				{
					...
					on: {
						click: function(event) {
							<% if(jp2MimeType.equalsIgnoreCase("image/jp2")) {%>
								location.href = '<%= DLUtil.getPreviewURL(jp2FileEntry,jp2FileEntry.getFileVersion(), themeDisplay, StringPool.BLANK) %>';
							<%} else { %>
